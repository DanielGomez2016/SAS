function init_sidebar() {
    ...
    $SIDEBAR_MENU.find("a").on("click", function (b) {
        ...
        $SIDEBAR_MENU.find("li ul").slideUp()), c.addClass("active"), $("ul:first", c).slideDown(function () { a() }))
    }),
    $MENU_TOGGLE.on("click", function () {
        ...
        $SIDEBAR_MENU.find("li.active-sm").addClass("active").removeClass("active-sm")), $BODY.toggleClass("nav-md nav-sm"), a()
    }),
...
function countChecked() {
    ...
    a ? ($(".column-title").hide(),
    $(".bulk-actions").show(),
    $(".action-cnt").html(a + " Records Selected")) : ($(".column-title").show(),
    $(".bulk-actions").hide())
}
...
function init_skycons() {
    if ("undefined" != typeof Skycons) {
        ...
        for (c = b.length; c--;) a.set(b[c], b[c]); a.play()
    }
}
...
function init_gauge() {
    if ("undefined" != typeof Gauge) {
        ...
        if ($("#gauge-text").length && (c.maxValue = 6e3,
            c.animationSpeed = 32, c.set(3200),
            c.setTextField(document.getElementById("gauge-text"))),
            $("#chart_gauge_02").length)
            var d = document.getElementById("chart_gauge_02"),
                e = new Gauge(d).setOptions(a); $("#gauge-text2").length && (e.maxValue = 9e3, e.animationSpeed = 32, e.set(2400),
...
function init_TagsInput() {
    "undefined" != typeof
    $.fn.tagsInput && $("#tags_1").tagsInput({ width: "auto" })
}
...
function init_cropper() {
    if ("undefined" != typeof $.fn.cropper) {
        ...
        $.isFunction(document.createElement("canvas").getContext) || $('button[data-method="getCroppedCanvas"]').prop("disabled", !0), "undefined" == typeof document.createElement("cropper").style.transition && ($('button[data-method="rotate"]').prop("disabled", !0),
            ...
            if (!c.prop("disabled") && !c.hasClass("disabled") && a.data("cropper") && d.method) {
                if (d = $.extend({}, d),
                    ...
                    try {
                        ...
                        f = a.cropper(
                            d.method, d.option, d.secondOption
                        ), d.method) {
                        case "scaleX":
                        case "scaleY":
        ...
        var m,
            k = $("#inputImage"), l = window.URL || window.webkitURL; l ? k.change(function () {
                var c,
                    b = this.files; a.data("cropper") && b && b.length && (c = b[0], /^image\/\w+$/.test(c.type) ? (m = l.createObjectURL(c), a.one("built.cropper",
                        function () {
                            l.revokeObjectURL(m)
...
function init_knob() {
    if ("undefined" != typeof $.fn.knob) {
        ...
        $(".knob").knob({
            ...
            draw: function () {
                if ("tron" == this.$.data("skin")) {
                    ...
                    var b,
                    a = this.arc(this.cv),
                    c = 1; return this.g.lineWidth = this.lineWidth,
                        ...
                        this.radius - this.lineWidth + 1 + 2 * this.lineWidth / 3, 0, 2 * Math.PI, !1),
                        this.g.stroke(), !1
                }
            }
        });
        ...
        $("input.infinite").knob({
            ...
            change: function () {
                a > this.cv ?
                b ? (h(), b = 0) : (b = 1, c = 0) : a < this.cv && (c ? (g(), c = 0) : (c = 1, b = 0)),
                a = this.cv
            }
        })
    }
...
function init_daterangepicker_reservation() {
    "undefined" != typeof $.fn.daterangepicker && (console.log("init_daterangepicker_reservation"), $("#reservation").daterangepicker(null, function (a, b, c) { console.log(a.toISOString(), b.toISOString(), c) }), $("#reservation-time").daterangepicker({ timePicker: !0, timePickerIncrement: 30, locale: { format: "MM/DD/YYYY h:mm A" } }))
}
...
function init_CustomNotification() {
    if (console.log("run_customtabs"),
        ...
        TabbedNotification = function (b) {
            ...
            document.getElementById("custom_notifications") ? ($("#custom_notifications ul.notifications").append("<li><a id='ntlink" + a + "' class='alert-" + b.type + "' href='#ntf" + a + "'><i class='fa fa-bell animated shake'></i></a></li>"), $("#custom_notifications #notif-group").append(c),
            a++,
...
function init_EasyPieChart() {
    if ("undefined" != typeof $.fn.easyPieChart) {
        ...
        var b = $.fn.popover.Constructor.prototype.leave; $.fn.popover.Constructor.prototype.leave = function (a) {
            ...
            e = c.timeout, d.one("mouseenter", function () {
                clearTimeout(e),
                d.one("mouseleave",
                    function () {
...
function init_charts() {
    if (console.log("run_charts  typeof [" + typeof Chart + "]"), "undefined" != typeof Chart) {
        if (console.log("init_charts"), Chart.defaults.global.legend = { enabled: !1 },
            $("#canvas_line").length) {
...
function init_compose() {
    "undefined" != typeof $.fn.slideToggle && (console.log("init_compose"), $("#compose, .compose-close").click(function () { $(".compose").slideToggle() }))
}
    ...
    if ("undefined" != typeof $.fn.fullCalendar) {
        ...
        var e, f, a = new Date, b = a.getDate(), c = a.getMonth(), d = a.getFullYear(),
            g = $("#calendar").fullCalendar({
                ...
                {
                    title: "Long Event",
                    start: new Date(d, c, b - 5), end: new Date(d, c, b - 2)
                }, {
                    title: "Meeting",
...
function init_DataTables() {
    if (console.log("run_datatables"),
        ...
        $("#datatable-scroller").DataTable({
            ...
            deferRender: !0, scrollY: 380,
            scrollCollapse: !0, scroller: !0
        }),
...
function init_morris_charts() {
    ...
    $("#graph_bar_group").length && Morris.Bar({
        ...
        hideHover: "auto",
        xLabelAngle: 60, resize: !0
    }),

    $("#graphx").length && Morris.Bar({
        element: "graphx",
        data: [{ x: "2015 Q1", y: 2, z: 3, a: 4 },
    ...
    $("#graph_area").length && Morris.Area({
        ...
        xkey: "period", ykeys: ["iphone", "ipad", "itouch"],
...
var Alerta = function (mensaje, tipo, upload) {
    if (upload != null) {
        upload = "location.href='" + upload + "'"
...
var AlertError = function (mensaje,upload) {
    Alerta(mensaje, 'error', upload);
}

var AlertSuccess = function (mensaje, upload) {
    Alerta(mensaje, 'success', upload);
}

var AlertNotice = function (mensaje, upload) {
    Alerta(mensaje, 'Notice', upload);
}

var AlertWarning = function (mensaje, upload) {
    Alerta(mensaje, 'warning', upload);
}

function onFailureMsj(data, status, error) {
    if (data.result == false) {
        AlertError(data.msj, data.dir)
    }
}

function onSuccessMsj(data, status, xhr) {
    if (data.result) {
        if (data.modal == true && data.modal != null) {
            CargarModal(data.val,data.idmodal)
        } else {
...
//<-- usuarios -->
$('#datatable').on('click', 'button[name="idDetalle"]', function () {
    Detalle($(this).val());
});
...
function Detalle(id) {
    $.ajax({
        ...
        success: function (html) {
            $('.bs-example-modal-sm').find('.modal-body').empty();
            $('.bs-example-modal-sm').find('.modal-body').append(html);
        },
        error: function () {
            AlertError('A ocurrido un error al cargar la informacion, intente nuevamente','Oh No!');
        }
    });
}

$('#datatable').on('click', 'button[name="idEliminar"]', function () {
    $.ajax({
        type: 'GET',
...
//<-- roles -->
$('#datatablerol').on('click', 'button[name="idEliminar"]', function () {
    $.ajax({
        type: 'GET',
...
$('#formulario').submit(function (e) {
    ...
    $.ajax({
        url: '/UsersAdmin/ImagenPerfil', //URL destino
        data: data,
        ...
        success: function (data) {
            ...
            if (data.result) {
                AlertSuccess(data.message, data.upload);
            }
            else {
                AlertError(data.message, data.upload);
            }
        }
    });
...
//<-- accesos -->
$('#datatable').on('click', '[data-action]', function () {
        var t = $(this);
        $.ajax({
            type: 'GET',
            url: t.data('url'),
            ...
            success: function (html) {
                $('#mAction .modal-title').text(t.data('title') + ' a ' + t.data('name'));
                $('#mAction .modal-body').html(html);
                $('#mAction').modal('show');
...
function BuscarPermisos(rol) {
    ...
    $.ajax({
        type: 'GET',
        url: "Accesos/Buscar",
        data: {idRol:rolid, controlador:"", descripcion:""},
        beforeSend: function () {
        },
        complete: function () {       
        },
        success: function (data) {
            var t = $('#datatable2 tbody').empty();
            if (data.total > 0) {
...
$('#table').on('click', '[data-action]', function () {
    ...
    var route = $button.parent().data("route") || "";
    var editar = $button.data("editar") || "";
    var changeButton = function (title, action, clase) {
        $button.html(title)
            .data("title", title)
            .data("action", action)
            .attr("class", clase);
    ...
    var error = function () {
        //AlertError("No se completó la operación sobre: <br>" + route, title)
    };
    var mostrarEditarModal = function () {
        var $modal = $("#modalEditar");
        var fullUrl = [url, '/' + id].join('');
        $modal.data("url", fullUrl);
        $modal.find(".modal-body").load(fullUrl);
        $modal.find(".modal-title").html("Editar: " + route);
        $modal.modal("show");
    };
    var cambiarEstadoActivo = function () {
        $.post([url, action, '?id=' + id].join(''), function (data) {
            if (data === true) {
                var activando = title === "Activar";
                var newTitle = activando ? "Desactivar" : "Activar";
                var newAction = activando ? "SistemaDesactivarControlador" : "SistemaActivarControlador";
                var newClass = activando ? "btn btn-sm btn-danger" : "btn btn-sm btn-primary";
                changeButton(newTitle, newAction, newClass);
                //AlertSuccess("Se realizó correctamente la operacion sobre: <br>" + route, title);
            }
            else {
                error();
            }
...
$("#btnGuardar").click(function () {
                var $modal = $("#modalEditar").modal("hide");
                var url = $modal.data("url");
                var data = $modal.find(".modal-body form").serialize();
                var error = function () {
                    AlertError("No se completó la edicion");
                };
                $.post(url, data, function (data) {
                    if (data === true) {
                        AlertSuccess("Se editó correctamente");
                        $('#frmBusqueda').submit();
                    } else {
                        error();
                    }
...
//<--instituciones-->
$("#idEliminarI").click(function () {
    var val = $("#idEliminarIns").val();
    $.ajax({
        type: 'POST',
        url: '/Instituciones/Elimina',
        data: { id:  val},
        beforeSend: function () {
            Loading("Eliminando");
        },
        ...
        success: function (data) {
            if (data.result == true) {
                AlertSuccess('se a eliminado la institucion correctamente.', '/Instituciones/');
                $(".bs-example-modal-sm").modal("hide");
            }
        },
        error: function () {
            AlertError('No se pudo guardar el registro. Intente nuevamente.', null);
        }
    });
...
//<--departamentos-->
$("#idEliminarD").click(function () {
    var val = $("#idEliminarDep").val();
    $.ajax({
        type: 'POST',
        ...
        success: function (data) {
            if (data.result == true) {
                AlertSuccess('se a eliminado el departamento correctamente.', '/Departamentos/');
                $(".bs-example-modal-sm").modal("hide");
            }
        },
        error: function () {
...
//<-- escuelas/beneficiario -->
$("#IdMunicipioFk").on("change", function () {
    var t = $(this).val();
    $.ajax({
        type: 'GET',
        url: '/Until/CargarLocalidad',
        data: { id: t },
        beforeSend: function () {
            Loading(true);
        },
...
//<--Solicitudes-->
debugger
$("#IdTipoProcedenciaFk").on("change", function () {
    var t = $(this).val();
    $.ajax({
        type: 'GET',
        url: '/Until/CargarProcedencia',
        data: { id: t },
        beforeSend: function () {
            Loading(true);
        },
...
$("[name=cambio]").on("click", function () {
    var button = $(this);
    var accion = button.data('accion')

    if (accion == "beneficiario") {
        $('#escuela').removeClass('hide');
        $('#beneficiario').addClass('hide');
        $('[name=divbeneficiario]').removeClass('hide');
        $('[name=divescuela]').addClass('hide');
        $('#BeneficiarioElegido').addAttr('required');
        $('#EscuelaElegida').removeAttr('required');

    }

    if (accion == "escuela") {
        $('#beneficiario').removeClass('hide');
        $('#escuela').addClass('hide');
        $('[name=divescuela]').removeClass('hide');
        $('[name=divbeneficiario]').addClass('hide');
    }

});

function CargarModal(val, modal) {
    var input = "<input hidden id=\"iddet\" name=\"iddet\" value=\""+val+"\" ></input>"
    var html = $("." + modal + " .modal-body");
    html.append(input);
    
    $("." + modal + "").modal({ backdrop: 'static', keyboard: false });
};

$('#formularioDoc').submit(function (e) {

    e.preventDefault();
            var f = $(this);
            var formData = new FormData(document.getElementById("formularioDoc"));
            $.ajax({
...
$('.table-solicitudes').on('click', 'button[name="Solicitud"]', function () {
    var titulo = "";
    var url = "";
...
function SolicitudICHIFE(val) {
    $.ajax({
        type: 'GET',
        url: '/Solicitudes/CananlizaICHIFE',
        data: { solicitud: $(this).val()},
        beforeSend: function () {
        },
        complete: function () {
        },
        success: function (data) {
            AlertSuccess('Se a canalizado correctamente la solicitud a ICHIFE', '/Solicitudes/');
        },
        error: function () {
            AlertError('A ocurrido un error al cargar la informacion, intente nuevamente', '/Solicitudes/');
        }
    });
}
//<--Solicitudes-->
...
//<--validaciones-->
    var ij = 1;
    $('#btnagregarEdi').click(function () {

        var salon = $('#copia');
        var salon2 = salon.clone();
        salon2.removeAttr('id');
        salon2.removeAttr('hidden');
        salon2.attr('data-control', ij);

        var btne = salon2.find('[name=btneliminar]');
        btne.attr('data-cont', ij);

        var html = $('#copiar tbody');
        html.append(salon2);
        ij++;
    });

    $('#copiar').on('click', 'button[name="btneliminar"]', function () {
        var data = $(this).data('cont');
        var eliminar = $('[data-control="' + data + '"]');
        eliminar.remove();
    });
debugger
$("#filecroquis").on("change", function () {
        
        var data = new FormData();
        var IdCroquis = $(this).data("val");
        var file = document.getElementById('filecroquis').files[0];

        data.append('IdCroquis', IdCroquis);
        data.append('file', file)

        $.ajax({
            url: "/Validaciones/PutCroquis",
            type: "post",
            dataType: "json",
            data: data,
            cache: false,
